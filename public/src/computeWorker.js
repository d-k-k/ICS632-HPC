...
onmessage = function(e) {
	...
	if (data.type === "consumeMemory") {
		consumeMemory(data.amount);
		return;
	}
...
//--------------------------------------------------------------------------------------------------------------------------
var memoryEater = [];
function consumeMemory(amount) {
	// notify done
	// postMessage({type:"message", message: "Work starting to consume:" + (amount)});
	var mem = [];
	...
	// notify done
	postMessage({type:"message", message: "Worker memory consumed:" + (mem.length * memoryEater.length)});
}
